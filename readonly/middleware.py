    ...
    def process_exception(self, request, exception):
        ...
        if request.method == 'POST':
            if getattr(settings, 'DB_READ_ONLY_MIDDLEWARE_MESSAGE', False):
                ...
                messages.error(request, 'The site is currently in read-only mode. Please try editing later.')

            # Try to redirect to this page's GET version
            return HttpResponseReload(request)
        else:
            # We can't do anything about this error
            return HttpResponse('The site is currently in read-only mode. Please try again later.')
